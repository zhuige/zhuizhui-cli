...
class Git {
  ...
  //git flow流程 自动提交
  async commit() {
    // release/x.y.z  dev/x.y.z 版本号 major/minor/patch
    //生成开发分支
  ...
  async initCommit() {
    ...
    if (await this.checkRemoteMaster()) {
      await this.pullRemoteReop('master', {
  ...
  async pullRemoteReop(branchName, options = {}) {
    log.info(`同步远程${branchName}分支`)
    let sp
    try {
      sp = spinnerStart('pull中...')
      await this.git.pull('origin', branchName, options)
      log.success(`同步远程分支成功`)
    } catch (error) {
      log.notice('pull 出错了，要是代码冲突了请手动解决冲突后重试')
      throw error
    } finally {
      sp.stop(true)
    }
